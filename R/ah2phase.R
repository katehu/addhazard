...
#' nwts.2ph$trel <- nwts.2ph$trel + runif(dim(nwts.2ph)[1],0,1)*1e-8
#' fit1 <- ah.2ph(Surv(trel,relaps) ~ age + histol, ties = F, data = nwts.2ph, R = in.ph2, Pi = Pi,
#'                                  robust = FALSE,  calibration.variables = NULL)
#' summary(fit1)
#'
#' ### fit an additve hazards model with calibration on age
#' fit2 <- ah.2ph(Surv(trel,relaps) ~ age + histol, ties = F, data = nwts.2ph, R = in.ph2, Pi = Pi,
...
#' nwts.2ph$age2 <- nwts.2ph$age^2
#' fit3 <- ah.2ph(Surv(trel,relaps) ~ age + histol, ties = F, data = nwts.2ph, R = in.ph2, Pi = Pi,
...
#' fit4 <- ah.2ph(Surv(trel,relaps) ~ age + histol, ties = F, data = nwts, R = in.ph2, Pi = Pi,
#'                                  robust = FALSE,  calibration.variables = c("strt1","strt2","strt3"))
#' summary(fit4)

ah.2ph <- function(formula, data, R, Pi, ties, robust = FALSE, calibration.variables = NULL, ...) {
